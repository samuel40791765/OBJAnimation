...
void main(void)
{
  ...
  // normalized and aspect ratio corrected pixel coordinate
	...
	vec3 oo = vec3(0, 0, 1.0-mouse.y/resolution.y);
	vec3 od = normalize(vec3(p.x, p.y, -2.0));
	vec3 o,d;	
	o.xz = rt(oo.xz, 6.3*mouse.x/resolution.x);
	o.y = oo.y;
	d.xz = rt(od.xz, 6.3*mouse.x/resolution.x);
	d.y = od.y;
	...
	{	
		...
		for (float i = 0.0; i < SLICES; i+=1.0)
		{
			...
			for (int j = 0; j < 3; j++)									
			{
				d += 0.5/abs((fbm(5.0*curpos*b+ANIMATION_SPEED*time/b)*2.0-1.0)/a);
				b *= 2.0;
				a /= 2.0;
			}
